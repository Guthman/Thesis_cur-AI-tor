...
class FeatureExtractor:
    ...
    @property
    def PIL_img(self):
        if self._PIL_img is None or self.reuse_loaded_image is False:
            try:
    ...
    @property
    def cv2_img(self):
        if self._cv2_img is None or self.reuse_loaded_image is False:
            try:
    ...
    def extract(self, img_path):
        self.reuse_loaded_image = False
        if self.img_path == img_path:
            self.reuse_loaded_image = True
